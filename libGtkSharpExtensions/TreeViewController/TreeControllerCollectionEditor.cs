...
namespace GtkSharp.Extensions
{
	...
	public class TreeControllerCollectionEditor<T> : ITreeControllerCollectionEditor where T : new()
	{
		...
		/// <summary>
		/// Gets or sets the combo column parsable.
		/// </summary>
		/// <value>The combo column parsable.</value>
		public Dictionary<string, bool> ComboColumnParseText { get; set; }

		public Dictionary<string, object> ComboColumnDefaultValue { get; set; }
		...
		{
			AddColumnDef (propertyName, columnHeader, displayer, parser, true);
		}

		public void AddColumnDef (string propertyName, string columnHeader, Func<object, string> displayer, Func<string, object> parser, bool parseText)
		{
			if (parser != null)
				ColumnParsers.Add (propertyName, parser);
			ColumnDisplayers.Add (propertyName, displayer);
			ColumnHeaders.Add (propertyName, columnHeader);
			ComboColumnParseText.Add (propertyName, parseText);
		}
		...
		{
			Func<string, TRef> parser = x => new TRef ();
			//parser = null;
			AddColumnDef<TRef> (propertyName, columnHeader, displayer, parser, false, selectionItems, selectionItemComparer, allowCustom);
		}
		...
		{
			Func<string, object> parser2 = null;
			if (parser != null)
				parser2 = x => parser (x);
			AddColumnDef (propertyName, columnHeader, x => displayer (x as TRef), parser2);
		...
		/// <param name="displayer">Displayer.</param>
		/// <param name="parser">Parser.</param>
		/// <param name="parseText">Whether to parse the text value of the combobox.</param>
		/// <param name="selectionItems">Selection items.</param>
		/// <param name="selectionItemComparer">Selection item comparer.</param>
		/// <param name="allowCustom">If set to <c>true</c> allow custom.</param>
		/// <typeparam name="TRef">The 1st type parameter.</typeparam>
		public void AddColumnDef<TRef> (string propertyName, string columnHeader
			, Func<TRef, string> displayer, Func<string, TRef> parser, bool parseText
			, System.Collections.IEnumerable selectionItems
			, System.Collections.IEqualityComparer selectionItemComparer
			, bool allowCustom) where TRef : class
		{
			...
			if (parser != null)
				parser2 = x => parser (x);
			AddColumnDef (propertyName, columnHeader, x => displayer (x as TRef), parser2, parseText);
		...
		protected TreeControllerCollectionEditor (IEqualityComparer<string> propertyNameComparer)
		{
			...
			this.ComboColumnParseText = new Dictionary<string, bool> (propertyNameComparer);
...
//			this.ColumnCustomValueAllowed = new List<string> ();
		...
		public void BuildTreeColumns (TreeView widget, out int uneditableColumnCount)
		{
			...
			foreach (var prop in props) {
				...
				if (IsSelectionProp (prop.Name, out columnValues, out allowCustom)) {
					...
					func = new TreeCellDataFunc (ComboCellFunction);
				} else {
		...
		void ComboCell_Edited (object o, EditedArgs args)
		{
			...
			if (_treeStore.GetIterFromString (out iter, path)) {
				...
				if (ColumnProperties.TryGetValue (cell.Handle.ToInt32 (), out prop1)) {
					...
					bool parseText;
					if (!ComboColumnParseText.TryGetValue (prop1.Name, out parseText))
						parseText = true;
					// This is a string value, so check for a parser
					// If the parser does not exist, then do nothing

					Func<string, object> parser;
					if (ColumnParsers.TryGetValue (prop1.Name, out parser) && parseText) {
						prop1.SetValue (item, parser (strVal), null);
					}
				}
			}
		}
		...
		void ComboCell_EditingStarted (object o, EditingStartedArgs args)
		{
			...
			if (combo != null) {
				...
				if (this.ColumnProperties.TryGetValue (cell.Handle.ToInt32 (), out prop)) {
					...
					bool parseText;
					if (!ComboColumnParseText.TryGetValue (prop.Name, out parseText))
						parseText = true;
		...
		/// <summary>
		/// A cell data function for a Combo cell
		/// </summary>
		/// <param name="tree_column">Tree column.</param>
		/// <param name="cell">Cell.</param>
		/// <param name="tree_model">Tree model.</param>
		/// <param name="iter">Iter.</param>
		void ComboCellFunction (TreeViewColumn tree_column, CellRenderer cell, TreeModel tree_model, TreeIter iter)
		{
			System.Reflection.PropertyInfo prop;
			if (ColumnProperties.TryGetValue (cell.Handle.ToInt32 (), out prop)) {
				object defaultValue = null;
				Func<string, object> parser;
				if (ColumnParsers.TryGetValue (prop.Name, out parser))
					defaultValue = parser ("");
				CellDataFunction (cell, tree_model, iter, defaultValue, (CellRenderer oCell, object value, string columnName) => ((CellRendererText)oCell).Text = GetColumnDisplay (value, columnName));
			}
		}
